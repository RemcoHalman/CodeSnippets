# Use this script to convert Qt Designer UI and resource files to python
import os
import subprocess

input_path = os.path.join(os.path.dirname(__file__))  # "MainWindowTest", "ui"
    ...
    if extension == ".ui":
        ...
        if os.path.isfile(output_file):
            if not check_file(output_file):
                print(
                    "Warning: tried to overwrite a file generated outside Qt Creator. {}".format(
                        output_file
                    )
                )
                continue
        subprocess.call(
            "pyuic5 --import-from=widgets -x {} -o {}".format(input_file, output_file),
            shell=True,
        )
    elif extension == ".qrc":
        input_file = os.path.join(input_path, f)
